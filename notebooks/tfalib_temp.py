...
def constant_cadence(t_obs, x_obs, sampling_rate, interp = False):
    ...
    >>> (t_rec, x_rec, nn) = constant_cadence(t_obs, x_obs, FS, False)
    >>> (t_int, x_int, nn) = constant_cadence(t_obs, x_obs, FS, True)
    >>> plt.figure(3)
...
def moving_mean_and_stdev(x, window_size, unbiased_std = True):
    ...
    >>> m, s = moving_mean_and_stdev(x, 50)
...
def moving_q25_and_q75(x, window_size):
    ...
    >>> t = np.linspace(0, 2*np.pi, N)
    >>> x = np.sin(2*np.pi*t/np.pi) + 0.1*np.random.randn(N)
    >>> q25, q75 = moving_q25_and_q75(x, 50)
    >>> iqr = q75 - q25
...
def outliers(x, window_size, method = 'iqr', multiplier = np.NaN):
    ...
    >>> outlier_inds = outliers(x, 25, method = 'iqr', multiplier = 1.5)
...
def morlet_wave(N=600, scale=1, dx=0.01, omega=6.203607835633639, roll=True, 
    ...
    >>> s_wave = 50
    >>> dx_wave = .5
    >>> m, m_x, m_norm = morlet_wave(N_wave, s_wave, dx_wave, roll=False, norm=True)
    >>> plt.figure()
...
def wavelet_transform(x, dx, minScale, maxScale, wavelet_function=morlet_wave, 
    ...
    >>> N = len(T)
    >>> dj=0.1
    >>> W, scales = wavelet_transform(X, 1/fs, morlet_wave, 2, 1000, dj)
    >>> Wsq = np.abs(W)**2
...
class TempTFAdata:
    def __init__(self, t, x, params=None):
        self.meta = {}
        self.D = t
        self.t = md.date2num(t)
    ...
    def image(self):
        ...
        for i in range(D):
            ...
            ax.contourf(self.d, freqs, np.log10(self.W[::, :, i]), 
                         cmap='jet', levels=np.linspace(m, x, 20), 
                         extend='min')
            ax.set_xlim(self.meta['General']['time_lims'])
    ...
    def wave_index(self):
        # identify start and end to chop off the padded section
        t0, t1 = self.params["time_lims"]
        i_start = self.D.get_loc(t0)
        i_end = self.D.get_loc(t1)
        I = np.sum(self.W[:,i_start:i_end,0], axis=0)
        return I
...
class Cadence(TFA_Process):
    ...
    def apply(self, target):
        target.t, target.X = constant_cadence(target.t, target.X, 
...
class Cleaning(TFA_Process):
    ...
    def apply(self, target):
        inds = outliers(target.X, self.params['Window_Size'], 
...
class Filtering(TFA_Process):
    ...
    def apply(self, target):
        target.X = filter(target.X, self.params['Sampling_Rate'], 
                                 self.params['Cutoff'])
        self.append_params(target)
...
class Wavelet(TFA_Process):
    ...
    def apply(self, target):
        ...
        N, D = target.X.shape
        target.s = wavelet_scales(self.params['Min_Scale'], 
        ...
        for i in range(D):
            wave = wavelet_transform(np.reshape(target.X[:,i], (N,)), 
                        dx = self.params['Time_Step'], 
                        minScale = self.params['Min_Scale'], 
                        maxScale = self.params['Max_Scale'], 
                        dj = self.params['dj'])[0]
            norm = wavelet_normalize(np.abs(wave)**2, target.s, 
                          dx = self.params['Time_Step'], 
                          dj = self.params['dj'], 
